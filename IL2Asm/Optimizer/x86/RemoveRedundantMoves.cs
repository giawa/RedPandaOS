    ...
    public class RemoveRedundantMoves
    {
        public static void ProcessAssembly(List<string> assembly)
        {
            for (int i = 0; i < assembly.Count - 1; i++)
            {
                ...
                if (nextInstruction.StartsWith("mov ebx, eax"))
                {
                    if (instruction.StartsWith("mov eax,"))
                    {
                        assembly[i] = "    mov ebx," + instruction.Substring(8);
                        assembly[i + 1] = ";" + assembly[i + 1];
                    }
                }
                else if (nextInstruction.StartsWith("cmp ebx,"))
                {
                    if (instruction.StartsWith("mov ebx,") && (instruction.EndsWith("x") || instruction.EndsWith("di")))
                    {
