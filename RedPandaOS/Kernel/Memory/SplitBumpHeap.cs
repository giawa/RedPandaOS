        ...
        public class PageAlignedHeap
        {
            ...
            public uint Malloc(uint size)
            {
                ...
                if (Used == null)
                {
                    ...
                    Used._array = Utilities.PtrToObject<uint[]>(this.Address + 4);  // 132 bytes (4 byte size of array plus the 128 entries)
                    CPU.WriteMemInt(this.Address + 4, 128); // put the size of the array in the first 4 bytes of the array (as if this created normally)

                    for (uint i = 2; i < 130; i++)
                        CPU.WriteMemInt(this.Address + (i << 2), 0);
                    //Array.Clear(Used._array, 0, Used._array.Length);
                    
                    for (int i = 0; i < 136 / 4; i++) Used[i] = true;
                    Free = 4096 - 136;
                }
